...
static const unsigned char CFGR0 = WDT | GPIO2 | GPIO1 | LVLPL | CELL10 | CDC_3;
...
static const unsigned char CFGR4 = 0x94;  		// Vuv = 2.808 Volts
// Vov = (Hex-32)*16*1.5mV
...
unsigned char LTC1_Read_Voltages(void)
{
	...
	for (i=0;i<=14;i++)	READ_CVR[i]=LTC1_exchange(0xFF);
...
// Validate communications
	...
	for (i=0;i<=14;i++) comp_PEC = Calculate_PEC(READ_CVR[i],comp_PEC);
	...
	for (i=0;i<=5;i++) // Read 12 voltage values
	{
...
unsigned char LTC2_Read_Voltages(void)
{
	...
	for (i=0;i<=14;i++)	READ_CVR[i]=LTC2_exchange(0xFF);
...
unsigned char LTC3_Read_Voltages(void)
{
	...
	for (i=0;i<=14;i++)	READ_CVR[i]=LTC3_exchange(0xFF);
